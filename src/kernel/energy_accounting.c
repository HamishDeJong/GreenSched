...
static uint
clamp_window(uint value, uint window)
{
...
void
green_stats_reset(struct green_proc_stats *stats, int pid)
{
...
void
green_stats_on_run_tick(struct green_proc_stats *stats)
{
  /* Ignore invalid input. */
  if(stats == 0)
    return;
...
void
green_stats_on_sleep_tick(struct green_proc_stats *stats)
{
  /* Ignore invalid input. */
  if(stats == 0)
    return;
...
void
green_stats_on_wakeup(struct green_proc_stats *stats)
{
  /* Ignore invalid input. */
  if(stats == 0)
    return;
...
void
green_stats_on_context_switch(struct green_proc_stats *stats)
{
  /* Ignore invalid input. */
  if(stats == 0)
    return;
...
void
green_stats_decay_recent_cpu(struct green_proc_stats *stats, uint decay_step)
{
  /* Ignore invalid input. */
  if(stats == 0)
    return;
...
uint
green_compute_energy_score(struct green_proc_stats *stats, int mode)
{
  uint wakeup_cost;
  uint switch_cost;
  ...
  wakeup_cost = mode == GREEN_MODE_ENERGY ? 4 : 3;
  switch_cost = mode == GREEN_MODE_ENERGY ? 3 : 2;

  stats->energy_score =
    stats->cpu_ticks * 5 +
    stats->wakeups * wakeup_cost +
    stats->context_switches * switch_cost +
    clamp_window(stats->recent_cpu, 100);
